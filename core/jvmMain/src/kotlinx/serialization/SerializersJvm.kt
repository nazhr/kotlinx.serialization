 ...
 */
@ExperimentalSerializationApi
public fun SerializersModule.serializer(type: Type): KSerializer<Any> = serializerByJavaTypeImpl(type, checkTypeArgs = true) ?: type.kclass().serializerNotRegistered()
 ...
 */
@ExperimentalSerializationApi
public fun SerializersModule.serializerOrNull(type: Type): KSerializer<Any>? = serializerByJavaTypeImpl(type, checkTypeArgs = false)

@OptIn(ExperimentalSerializationApi::class)
private fun SerializersModule.serializerByJavaTypeImpl(type: Type, checkTypeArgs: Boolean = true): KSerializer<Any>? =
    when (type) {
        is GenericArrayType -> {
            genericArraySerializer(type, checkTypeArgs)
        }
        is Class<*> -> typeSerializer(type, checkTypeArgs)
        is ParameterizedType -> {
            val rootClass = (type.rawType as Class<*>)
            val args = (type.actualTypeArguments)
            val argsSerializers = if (checkTypeArgs) args.map { serializer(it) } else args.map { serializerOrNull(it) ?: return null }
            when {
                List::class.java.isAssignableFrom(rootClass) -> ListSerializer(argsSerializers[0]) as KSerializer<Any>
...
@OptIn(ExperimentalSerializationApi::class)
private fun SerializersModule.typeSerializer(type: Class<*>, checkTypeArgs: Boolean): KSerializer<Any>? {
    return if (!type.isArray) {
        reflectiveOrContextual(type.kotlin as KClass<Any>)
    } else {
        val eType: Class<*> = type.componentType
        val s = if (checkTypeArgs) serializer(eType) else (serializerOrNull(eType) ?: return null)
...
private fun SerializersModule.genericArraySerializer(
    ...
    val serializer = if (checkTypeArgs) serializer(eType) else (serializerOrNull(eType) ?: return null)
