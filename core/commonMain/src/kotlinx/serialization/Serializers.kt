...
public fun SerializersModule.serializer(type: KType): KSerializer<Any?> =
    serializerByKTypeImpl(type, checkTypeArgs = true) ?: type.kclass().platformSpecificSerializerNotRegistered()
...
public fun SerializersModule.serializerOrNull(type: KType): KSerializer<Any?>? {
    return serializerByKTypeImpl(type, checkTypeArgs = false)
}

@OptIn(ExperimentalSerializationApi::class)
private fun SerializersModule.serializerByKTypeImpl(type: KType, checkTypeArgs: Boolean): KSerializer<Any?>? {
    ...
    val result: KSerializer<Any>? = when {
        ...
        else -> builtinSerializer(typeArguments, rootClass, checkTypeArgs)
    }?.cast()
...
private fun SerializersModule.builtinSerializer(
    ...
    val serializers = if (checkTypeArgs)
